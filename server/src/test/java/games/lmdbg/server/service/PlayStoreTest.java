...
class PlayStoreTest {
	...
	void testReadErrors() {
		...
		Mockito.doAnswer(invocation -> {
			...
			return null;
		}).when(this.mockJdbc).query(ArgumentMatchers.anyString(),
				ArgumentMatchers.any(RowCallbackHandler.class), ArgumentMatchers.eq( -1l));
	...
	void testWriteErrors() {
		...
		String componentString = "INSERT INTO " + Schema.COMPONENT_TABLE
				+ " (play_id, c_type, component_id) " + " VALUES (?, ?, ?)";
		String starterString = "INSERT INTO " + Schema.STARTER_TABLE
				+ " (play_id, starter_id, quantity) " + " VALUES (?,?,?)";
		...
		Exception thrown =
				Assertions.assertThrows(RuntimeException.class, () -> underTest.createPlay(testPlay));
